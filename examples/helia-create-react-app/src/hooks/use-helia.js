 ...
 *
 */
export default function useHelia (ipfs, cmd, opts) {
  const [res, setRes] = useState(null)
  useEffect(() => {
    callHelia(ipfs, cmd, setRes, opts)
  }, [ipfs, cmd, opts])
  return res
}

async function callHelia (ipfs, cmd, setRes, ...opts) {
  if (!ipfs) return null
  console.log(`Call helia.${cmd}`)
  const heliaCmd = getProperty(ipfs, cmd)
  const res = await heliaCmd(...opts)
  console.log(`Result helia.${cmd}`, res)
  setRes(res)
}
