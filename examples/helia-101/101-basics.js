...
// which is useful for simple use cases, where you don't need p2p networking to
// provide data to other nodes. Since this example is focused on UnixFS without
// p2p networking, we can use the `@helia/http` package.
...
// Use the TextEncoder to turn strings into Uint8Arrays to add to the node
const encoder = new TextEncoder()

// addBytes takes raw bytes and returns a raw block CID for the content
// binary blobs over 1 MiB are chunked and return a dag-pb block CID instead
// The `bytes` value we have passed to `unixfs` has now been turned into a
// UnixFS DAG and stored in the helia node.
...
// addFile returns a directory CID, retaining the filename derived from
// the `path` argument
...
// stat returns a UnixFSStats object containing information about the directory
const readmeStats = await fs.stat(readmeCid)
console.log('README.md stats:', readmeStats)

// To get the size of a directory, we need extended stats (to traverse the DAG)
...
// Helia will download, and add the file into smaller chunks and return
// a directory containing a file node `2600-h.htm` with links to the raw
// blocks of the file
...
// Instead of loading the file into memory like we did above, we can use
// the `cat` API, which returns an async iterable, allowing us to stream
// the file to a writable stream, which we can pipe to devNull,
// process.stdout, or a file.
try {
  await pipeline(
    fs.cat(urlCid, {
      path: '/2600-h.htm'
    ...
    // createWriteStream('./war_and_peace.html'),
  )
} catch (err) {
